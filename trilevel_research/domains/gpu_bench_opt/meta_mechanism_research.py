"""Level 3 meta-mechanism research for gpu_bench — patches mechanism_research.py."""
...
new GPU benchmark search mechanisms. Improve mechanism_research.py (Level 2) that patches
GpuBenchRunner."""
...
For EACH hypothesis:
1. **Target**: method/class to modify
2. **Core idea**: one sentence
3. **Why it helps Level 2**: causal argument
4. **Implementation complexity**: 1–5
5. **Risk**: low / medium / high

Allowed targets ONLY in mechanism_research.py.
"""

SPECIFY_SYSTEM = """You are a senior engineer writing a patch spec for mechanism_research.py."""
...
Patches domains/gpu_bench_opt/mechanism_research.py.
"""


class GpuBenchMetaMechanismResearcher(BaseMetaMechanismResearcher):
    """Level-3 researcher for gpu_bench."""

    RESEARCHER_CLASS_NAME = "GpuBenchMechanismResearcher"

    def _get_researcher_class_name(self) -> str:
        return self.RESEARCHER_CLASS_NAME

    def _get_mechanism_research_path(self) -> Path:
        return Path(__file__).parent / "mechanism_research.py"

    def _get_explore_prompt(self, **kwargs) -> tuple[str, str]:
        return (
            EXPLORE_PROMPT.format(
                l2_trace_summary=kwargs["l2_trace_summary"],
                mech_summary=kwargs["mech_code_summary"],
    ...
    def _get_specify_prompt(self, selected_hypothesis: str, critique: str, **kwargs) -> tuple[str, str]:
        return (
            SPECIFY_PROMPT.format(
                ...
                mech_section=kwargs.get("mechanism_research_section", "")[:3000],
            ),
            SPECIFY_SYSTEM,
        )
    ...
    def _get_reference_code(self, **kwargs) -> str:
        ...
        idx = code.find("class GpuBenchMechanismResearcher:")
        return code[idx : idx + 1500] if idx != -1 else code[:1000]

    def _parse_spec_metadata(self, spec: str, session_id: str) -> tuple[str, str, str]:
        patch_name = f"gpu_meta_patch_{session_id}"
